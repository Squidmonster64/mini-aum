...
describe("Mini AUM web studio beta contract", () => {
  ...
  it("exposes explicit audio sources and real loading controls", () => {
    ...
    expect(mixer).toContain("LOAD AUDIO");
  ...
  it("uses permissively licensed audio and MIDI foundations", () => {
    expect(mixer).toContain("tone@15.1.22");
    expect(mixer).toContain("@tonejs/midi@2.0.28");
    expect(mixer).toContain("Tone.Player");
    expect(mixer).toContain("Tone.PolySynth");
    expect(mixer).toContain("new Midi(data)");
  });
  ...
  it("has a functional MIDI editor surface", () => {
    ...
    expect(mixer).toContain("dblclick");
  });
